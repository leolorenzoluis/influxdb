...
func TestDurationMarshaling(t *testing.T) {
	t.Run("unmarshaling", func(t *testing.T) {
		...
		dur3 := options.Duration{}
		if err := dur3.UnmartialJSON([]byte("\"1h10m3s\"")); err != nil {
			t.Fatal(err)
		}

		d3, err3 := dur3.DurationFrom(now)
		if err3 != nil {
			t.Fatal(err3)

		}
		if d1 != d2 || d2 != d3 || d1 != time.Hour+10*time.Minute+3*time.Second {
	...
	t.Run("marshaling", func(t *testing.T) {
		...
		text, err = dur.MartialJSON()
		if err != nil {
			t.Fatal(err)
		}
		if string(text) != "\"1h10m3s\"" {
			t.Fatalf("duration json marshal should be \"\"1h10m3s\"\" but was %s", text)
		}
	})
...
func TestDurationMath(t *testing.T) {
	...
	if d != 10*time.Second {
		t.Fatal("expected duration to be 10s but it was ")
	}
}
